...
# sam = sam_model_registry["vit_b"](checkpoint="/home/lc/models/sam/sam_vit_b_01ec64.pth")
# predictor = SamPredictor(sam)
# predictor.set_image("../YOLO/zidane.jpg")
# masks, _, _ = predictor.predict(<input_prompts>)
# masks, _, _ = predictor.predict()

# 输入图片
sam_image = cv2.imread('../YOLO/zidane.jpg')
sam_image = cv2.cvtColor(sam_image, cv2.COLOR_BGR2RGB)
# 输入模型
sam_checkpoint = '/home/lc/models/sam/sam_vit_h_4b8939.pth'
# 输入模型类型
...
class SamPredict:
    ...
    def __init__(self, image:any, checkpoint:any, model_type:str, model_device:str, point_x: int, point_y: int, random_color: bool):
        self.image = image
    ...
    def show_pre(self) -> None:
        ...
        plt.scatter(self.point_x, self.point_y, color='red', marker='*', s=self.marker_size, edgecolor='white', linewidth=1.25)
        plt.axis('off')
        plt.show()
    ...
    def get_result(self) -> None:
        ...
        for i, (mask, score) in enumerate(zip(masks, scores)):
            plt.imshow(self.image)
            # 显示覆盖
            mask_image = self.__show_mask(mask)
            result_file = "result_" + str(i) + ".jpg"
            plt.imsave(result_file, mask_image)
            # mask_image.save(result_file)
            # 显示标记点
